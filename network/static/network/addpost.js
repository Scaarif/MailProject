	...
	if (text.length != 0) {
		...
		.then((res) => {
			if (res.status == 201) {
				...
				text_f.value = ""; //clear the add-post textarea for next new post
			}
		});
	}
});
...
function add_html(id, username, post, time, link) {
	...
	div6.appendChild(img); //the likes div
	div6.appendChild(span5);

	div5.appendChild(span2); //timestamp
	div5.appendChild(span3); //edit button

	a1.appendChild(span1); //username
